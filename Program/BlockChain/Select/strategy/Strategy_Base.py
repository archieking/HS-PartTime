...
# 取出策略文件名中_后的部分
stg_name = os.path.basename(__file__).split('.')[0].split('Strategy_')[1]
...
factor_list = [
    ('factor_name', True, 168, 1)  # 因子名（和factors文件中相同），排序方式，参数，权重。
]

# 确认过滤因子及其参数，用于`2_选币_单offset.py`进行过滤
filter_list = [
    # ('PctChange', 24)  # 因子名（和factors文件中相同），参数
]
...
def after_merge_index(df, symbol, factor_dict, data_dict):
    ...
    本函数住要的作用如下：
            ...
            2、合并外部数据，并指定外部数据在resample时的规则。例如：（伪代码）
                    ...
                    draw_df = pd.merge(draw_df,chain_df,'left','candle_begin_time') # 将链上数据合并到日线上
