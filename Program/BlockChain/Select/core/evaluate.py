...
def strategy_evaluate(equity, net_col='多空资金曲线', pct_col='本周期多空涨跌幅'):
    ...
    # 处理年度数据
    year_return['收益%'] = year_return[pct_col]
    year_return['累积净值'] = year_return['收益%'] + 1
    dd2here_list = []
    equity['year'] = equity['candle_begin_time'].dt.year
    for year, group_df in equity.groupby('year'):
        dd2here_list.append(float(group_df.sort_values(by=['净值dd2here']).iloc[0][['净值dd2here']]))
    year_return['最大回撤'] = dd2here_list
    year_return['收益回撤比'] = year_return['收益%'] / abs(year_return['最大回撤'])

    # 处理月度数据
    month_return['收益%'] = month_return[pct_col]
    month_return['累积净值'] = month_return['收益%'] + 1
    dd2here_list = []
    equity['month'] = equity['candle_begin_time'].dt.to_period('M')  # 提取年月
    for year, group_df in equity.groupby('month'):
        dd2here_list.append(float(group_df.sort_values(by=['净值dd2here']).iloc[0][['净值dd2here']]))
    month_return['最大回撤'] = dd2here_list
    month_return['收益回撤比'] = month_return['收益%'] / abs(month_return['最大回撤'])

    # 处理季度数据
    quarter_return['收益%'] = quarter_return[pct_col]
    quarter_return['累积净值'] = quarter_return['收益%'] + 1
    dd2here_list = []
    equity['quarter'] = equity['candle_begin_time'].dt.to_period('Q')  # 提取年季度
    for year, group_df in equity.groupby('quarter'):
        dd2here_list.append(float(group_df.sort_values(by=['净值dd2here']).iloc[0][['净值dd2here']]))
    quarter_return['最大回撤'] = dd2here_list
    quarter_return['收益回撤比'] = quarter_return['收益%'] / abs(quarter_return['最大回撤'])
