...
def step5_aggregate_select_results(conf: BacktestConfig, save_final_result=False):
    ...
    print()  # 命令行看起来更棒
    logger.warning(f'{conf.name}选币结果df大小：'
...
def simu_timing(conf: BacktestConfig, df_spot_ratio, df_swap_ratio, pivot_dict_spot, pivot_dict_swap):
    ...
    account_df, rtn, year_return, month_return, quarter_return = calc_equity(
        conf, pivot_dict_spot, pivot_dict_swap, df_spot_ratio, df_swap_ratio, leverages
    )
...
def run_backtest_multi(factory: BacktestConfigFactory):
    ...
    # ====================================================================================================
    s_time = time.time()
    logger.info(f'选币（时间久，和包含的策略及子策略数量、选币数量有关）...')
    # ** 正常回测**
    for conf in factory.config_list:
        logger.debug(f'回测{conf.name}的{len(conf.strategy_list)}个子策略，并行任务数：{job_num}')
        select_coins(conf)

    # ** 激进型 **
    # 电脑比较强悍的话，这个可以进一步释放进程撕裂者的潜力
    ...
    # 5. 针对选币结果进行聚合、回测模拟
    # ====================================================================================================
    logger.info(f'回测模拟（时间会比较久，主要和选币数量有关）...')
    s_time = time.time()
    report_list = []
    ...
    #     for future in tqdm(as_completed(futures), total=len(conf_list), desc='回测模拟'):
    #         try:
...
def find_best_params(factory: BacktestConfigFactory, show_heat_map=False):
    r_time = time.time()
    divider('遍历开始', '*')
    report_list = run_backtest_multi(factory)
    # ====================================================================================================
    # 6. 根据回测参数列表，展示最优参数
    # ====================================================================================================
    s_time = time.time()
    ...
    all_params_map.to_excel(factory.result_folder / f'最优参数.xlsx', index=False)
